...
public class RomanToInt {
    ...
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < cs.length; i++) {
            if (stack.isEmpty() || compare(cs[i], stack.peek()) > 0) {
                stack.push(cs[i]);
            } else {

                result += calRomanNum(stack);
                stack.push(cs[i]);
            }
        }
        result += calRomanNum(stack);
    ...
    //计算栈中的罗马数字，例 IX->9
    private int calRomanNum(Stack<Character> stack) {
        int temp = stack.isEmpty() ? 0 : ROMAN_MAP.get(stack.pop());
        while (!stack.isEmpty()) {
            temp -= ROMAN_MAP.get(stack.pop());
        }
        return temp;
    }

    private int compare(char x, char y) {
        return ROMAN_MAP.get(x) - ROMAN_MAP.get(y);
    }
