...
public class MyAtoi {
    ...
    public int myAtoi(String str) {
        ...
        if (cs[0] == '+' || cs[0] == '-') {
            ...
            return 0;
        int maxlimit = Integer.MAX_VALUE / 10, minlimit = Integer.MIN_VALUE / 10;
        for (; i < cs.length; i++) {
            if (cs[i] < '0' || cs[i] > '9')
                break;
            int cur = cs[i] - '0';
            if (flag == 1) {
                if (result < maxlimit || (result == maxlimit && cur <= 7))
                    result = result * 10 + cur;
                else
                    return Integer.MAX_VALUE;
            } else {
                if (result > minlimit || (result == minlimit && cur <= 8))
                    result = result * 10 - cur;
                else
    ...
    public static void main(String[] args) {
        ...
        System.out.println(myAtoi.myAtoi("   -42 with"));
